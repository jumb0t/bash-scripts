...
# 2. Получает список репозиториев через GitHub API (включая приватные), исключая форки.
...
# Получение списка репозиториев через GitHub API, исключая форки
function fetch_repos() {
  echo "Получение списка репозиториев для пользователя $GITHUB_USER..."
  curl -s -H "Authorization: token $GITHUB_TOKEN" \
    "https://api.github.com/user/repos?per_page=100" | \
    jq -r '.[] | select(.fork == false) | .clone_url' >"$REPO_LIST_FILE"
  ...
  echo "Список репозиториев (без форков) сохранён в файл: $REPO_LIST_FILE"
}
